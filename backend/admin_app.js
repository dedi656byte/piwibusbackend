...
function tripSocialSummary(trip) {
  return `
    <span class="social-pill">Likes ${number(tripLikeCount(trip))}</span>
    <span class="social-pill">Chat ${number(tripMessageCount(trip))}</span>
  `;
}
...
function renderBackend(data) {
  ...
  const container = $("backendMetrics");
  container.replaceChildren(
    ...[
...
function renderAttention(data) {
  ...
  list.replaceChildren(
    ...items.map((item) => {
      ...
      row.className = `attention-item ${item.level}`;
      ...
      const hintEl = document.createElement("small");
      hintEl.textContent = String(item.hint);

      row.append(labelEl, valueEl, hintEl);
      return row;
    }),
  );
}
...
function renderQualityAndDevices(data) {
  ...
  quality.replaceChildren(
    ...[
      ["Note moyenne", ratings.totalRatings ? `${number(ratings.average, 1)} / 5` : "-"],
      ["Avis recus", `${number(ratings.totalRatings)} sur ${number(ratings.ratedTrips)} trajet(s)`],
  ...
  const devicesList = $("deviceMetrics");
  devicesList.replaceChildren(
    ...[
      ["Appareils actifs", `${number(notifications.activeDevices)} / ${number(notifications.registeredDevices)}`],
...
function renderPriorityLines(data) {
  ...
  if (!lines.length) {
    ...
    empty.textContent = "Aucune ligne prioritaire pour le moment.";
  ...
  container.replaceChildren(
    ...lines.map((line) => {
      ...
      const row = document.createElement("div");
      row.className = "priority-line";

      const main = document.createElement("div");
      main.className = "priority-line-main";
      const pill = createLinePill(line.displayCode, line.colorValue);
      const stack = document.createElement("div");
      const title = document.createElement("strong");
      title.textContent = text(line.title || `Ligne ${line.displayCode || "-"}`);
      const subtitle = document.createElement("span");
      subtitle.textContent = `${number(line.activeTrips)} live - ${number(line.reports)} alerte(s) - ${number(line.observers)} observateur(s)`;
      stack.append(title, subtitle);
      main.append(pill, stack);

      const side = document.createElement("div");
      side.className = "priority-line-side";
      const distance = document.createElement("strong");
      distance.textContent = distanceKm(line.distanceKm);
      const meter = document.createElement("div");
      meter.className = "priority-meter";
      const fill = document.createElement("span");
      fill.style.width = `${width}%`;
      meter.appendChild(fill);
      side.append(distance, meter);
...
function renderTrips(trips) {
  ...
  table.replaceChildren(
    ...(!trips.length
      ? [emptyRow(10, "Aucun trajet live actif.")]
      : trips.flatMap((trip) => {
          ...
          cell1.append(createLinePill(trip.displayCode, trip.colorValue));
          const lineTitle = document.createElement("small");
          lineTitle.textContent = text(trip.lineTitle);
          cell1.append(document.createElement("br"), lineTitle);

          const cell2 = document.createElement("td");
          const ownerName = document.createElement("span");
          ownerName.textContent = text(trip.ownerName);
          const lastUpdated = document.createElement("small");
          lastUpdated.textContent = dateTime(trip.lastUpdatedAt);
          cell2.append(ownerName, document.createElement("br"), lastUpdated);

          const cell3 = document.createElement("td");
          cell3.textContent = number(trip.observers);
          const cell4 = document.createElement("td");
          cell4.textContent = `${number(trip.speedKmh, 1)} km/h`;
          const cell5 = document.createElement("td");
          cell5.append(document.createTextNode(distanceKm(trip.distanceKm)));
          cell5.append(document.createElement("br"));
          const pct = document.createElement("small");
          pct.textContent = `${number((trip.progress || 0) * 100)}%`;
          cell5.append(pct);
          const cell6 = document.createElement("td");
          const gpsCell = document.createElement("span");
          gpsCell.textContent = gpsLabel;
          const gpsMeta = document.createElement("small");
          gpsMeta.textContent = `${text(gps.accuracy, "-")} m - ${gpsAge}`;
          cell6.append(gpsCell, document.createElement("br"), gpsMeta);
          const cell7 = document.createElement("td");
          cell7.append(document.createTextNode(`${number(trip.ratingAverage, 1)} / 5`));
          cell7.append(document.createElement("br"));
          const usage = document.createElement("small");
          usage.textContent = dataSize(trip.networkUsageBytes);
          cell7.append(usage);
          const cell8 = document.createElement("td");
          cell8.append(...tripSocialSummaryGrip(trip));
          const cell9 = document.createElement("td");
          const status = document.createElement("span");
          status.className = `status-pill ${statusClass}`;
          status.textContent = trip.offRouteSince ? "hors trace" : "actif";
          const sys = document.createElement("small");
          sys.textContent = text(trip.lastSystemMessage);
          cell9.append(status, document.createElement("br"), sys);

          const cell10 = document.createElement("td");
          cell10.className = "row-actions";
          const detailsBtn = document.createElement("button");
          detailsBtn.className = "ghost-button";
          detailsBtn.type = "button";
          detailsBtn.dataset.tripDetails = trip.id;
          detailsBtn.setAttribute("aria-expanded", String(expanded));
          detailsBtn.textContent = expanded ? "Masquer" : "Details";
          const stopBtn = document.createElement("button");
          stopBtn.className = "danger-button";
          stopBtn.type = "button";
          stopBtn.dataset.stopTrip = trip.id;
          stopBtn.textContent = "Arreter";
          cell10.append(detailsBtn, stopBtn);

          row.append(cell1, cell2, cell3, cell4, cell5, cell6, cell7, cell8, cell9, cell10);
          return [row, ...(expanded ? [tripDetailRow(trip, 10)] : [])];
        })),
  );
}
...
function renderTripHistory(trips) {
  ...
  table.replaceChildren(
    ...(!completed.length
      ? [emptyRow(11, "Aucune diffusion terminee.")]
      : completed.flatMap((trip) => {
          const expanded = state.expandedTrips.has(trip.id);
          const row = document.createElement("tr");
          const cell1 = document.createElement("td");
          cell1.append(createLinePill(trip.displayCode, trip.colorValue));
          const title = document.createElement("small");
          title.textContent = text(trip.lineTitle);
          cell1.append(document.createElement("br"), title);

          const cell2 = document.createElement("td");
          const owner = document.createElement("span");
          owner.textContent = text(trip.ownerName);
          const role = document.createElement("small");
          role.textContent = text(trip.ownerRole);
          cell2.append(owner, document.createElement("br"), role);

          const cell3 = document.createElement("td");
          const started = document.createElement("span");
          started.textContent = dateTime(trip.startedAt);
          const ended = document.createElement("small");
          ended.textContent = dateTime(trip.lastUpdatedAt);
          cell3.append(started, document.createElement("br"), ended);

          const cell4 = document.createElement("td");
          cell4.textContent = durationLabel(trip.startedAt, trip.lastUpdatedAt);

          const cell5 = document.createElement("td");
          cell5.append(document.createTextNode(distanceKm(trip.distanceKm)));
          cell5.append(document.createElement("br"));
          const progress = document.createElement("small");
          progress.textContent = `${number((trip.progress || 0) * 100)}%`;
          cell5.append(progress);

          const cell6 = document.createElement("td");
          cell6.append(document.createTextNode(`max ${number(trip.maxObservers)}`));
          cell6.append(document.createElement("br"));
          const current = document.createElement("small");
          current.textContent = `${number(trip.observers)} actuel(s)`;
          cell6.append(current);

          const cell7 = document.createElement("td");
          cell7.textContent = dataSize(trip.networkUsageBytes);
          const cell8 = document.createElement("td");
          cell8.append(document.createTextNode(`${number(trip.ratingAverage, 1)} / 5`));
          cell8.append(document.createElement("br"));
          const reviews = document.createElement("small");
          reviews.textContent = `${number(trip.ratingCount)} avis`;
          cell8.append(reviews);
          const cell9 = document.createElement("td");
          cell9.append(...tripSocialSummaryGrip(trip));
          const cell10 = document.createElement("td");
          const status = document.createElement("span");
          status.className = `status-pill ${trip.offRouteSince ? "danger" : ""}`;
          status.textContent = text(trip.status);
          const systemMessage = document.createElement("small");
          systemMessage.textContent = text(trip.lastSystemMessage);
          cell10.append(status, document.createElement("br"), systemMessage);
          const cell11 = document.createElement("td");
          cell11.className = "row-actions";
          const detailsBtn = document.createElement("button");
          detailsBtn.className = "ghost-button";
          detailsBtn.type = "button";
          detailsBtn.dataset.tripDetails = trip.id;
          detailsBtn.setAttribute("aria-expanded", String(expanded));
          detailsBtn.textContent = expanded ? "Masquer" : "Details";
          cell11.append(detailsBtn);

          row.append(cell1, cell2, cell3, cell4, cell5, cell6, cell7, cell8, cell9, cell10, cell11);
          return [row, ...(expanded ? [tripDetailRow(trip, 11)] : [])];
        })),
  );
}
...
function renderUsers(users) {
  ...
  table.replaceChildren(
    ...(!visible.length
      ? [emptyRow(6, "Aucun utilisateur.")]
      : visible.map((user) => {
          const row = document.createElement("tr");
          const cell1 = document.createElement("td");
          cell1.textContent = text(user.fullName);
          if (user.connected) {
            const pill = document.createElement("span");
            pill.className = "status-pill";
            pill.textContent = "connecte";
            cell1.append(" ", pill);
          }
          const cell2 = document.createElement("td");
          cell2.append(document.createTextNode(text(user.email)));
          cell2.append(document.createElement("br"));
          const phone = document.createElement("small");
          phone.textContent = text(user.phone);
          cell2.append(phone);
          const cell3 = document.createElement("td");
          ...
          status.textContent = text(user.status);
          cell4.append(status);
          const cell5 = document.createElement("td");
          cell5.append(document.createTextNode(dateTime(user.lastSeenAt)));
          cell5.append(document.createElement("br"));
          const session = document.createElement("small");
          session.textContent = `${number(user.sessionCount)} session(s)`;
          cell5.append(session);
          const cell6 = document.createElement("td");
          cell6.className = "row-actions";
...
function renderReports(reports) {
  ...
  table.replaceChildren(
    ...(!reports.length
      ? [emptyRow(7, "Aucune alerte.")]
      : reports.map((report) => {
          const row = document.createElement("tr");
          const cell1 = document.createElement("td");
          cell1.append(document.createTextNode(text(report.lineCode)));
          cell1.append(document.createElement("br"));
          const label = document.createElement("small");
          label.textContent = text(report.lineLabel);
          cell1.append(label);
          const cell2 = document.createElement("td");
          cell2.textContent = text(report.busNumber);
          const cell3 = document.createElement("td");
          cell3.append(document.createTextNode(text(report.reporterName)));
          cell3.append(document.createElement("br"));
          const created = document.createElement("small");
          created.textContent = dateTime(report.createdAt);
          cell3.append(created);
          const cell4 = document.createElement("td");
          const status = document.createElement("span");
          status.className = `status-pill ${report.status === "refuse" ? "danger" : report.status === "en_attente" ? "warn" : ""}`;
          status.textContent = text(report.status);
          cell4.append(status);
          const cell5 = document.createElement("td");
          cell5.textContent = percent(report.confidence, 0);
          const cell6 = document.createElement("td");
          cell6.textContent = text(report.note);
          const cell7 = document.createElement("td");
          cell7.className = "row-actions";
          const validateBtn = document.createElement("button");
          validateBtn.type = "button";
          validateBtn.dataset.reportStatus = report.id;
          validateBtn.dataset.status = "valide";
          validateBtn.textContent = "Valider";
          const refuseBtn = document.createElement("button");
          refuseBtn.type = "button";
          refuseBtn.className = "danger-button";
          refuseBtn.dataset.reportStatus = report.id;
          refuseBtn.dataset.status = "refuse";
          refuseBtn.textContent = "Refuser";
          cell7.append(validateBtn, refuseBtn);
...
function renderCatalog(data) {
  ...
  layersList.replaceChildren(
    ...layers.map((layer) => {
      ...
      name.textContent = text(layer.name);
      const scope = document.createElement("span");
      scope.textContent = text(layer.scope);
      details.append(name, document.createElement("br"), scope);
      const source = document.createElement("span");
      source.textContent = text(layer.source);
      item.append(details, source);
      return item;
    }),
  );
  ...
  table.replaceChildren(
    ...(!lines.length
      ? [emptyRow(6, "Aucune ligne.")]
      : lines.map((line) => {
          const row = document.createElement("tr");
          const cell1 = document.createElement("td");
          cell1.append(createLinePill(line.displayCode, line.colorValue));
          const cell2 = document.createElement("td");
          cell2.textContent = text(line.title);
          const cell3 = document.createElement("td");
          cell3.textContent = `${number(line.trips)} dont ${number(line.activeTrips)} actif(s)`;
          const cell4 = document.createElement("td");
          cell4.textContent = distanceKm(line.distanceKm);
          const cell5 = document.createElement("td");
          cell5.textContent = number(line.reports);
          const cell6 = document.createElement("td");
          cell6.textContent = number(line.observers);
          row.append(cell1, cell2, cell3, cell4, cell5, cell6);
          return row;
        })),
  );
...
function populateMapLineFilter() {
  ...
  allOption.value = "";
  allOption.textContent = "Toutes les lignes";
  select.appendChild(allOption);

  lines.forEach((line) => {
    const option = document.createElement("option");
    option.value = text(line.lineCode);
    option.textContent = `${text(line.displayCode)} - ${text(line.title || "Itineraire")}`;
...
function renderMapSidePanel(liveTrips, visibleLines, liveUsers, showUsers = true) {
  ...
  if (!liveTrips.length) {
    liveList.replaceChildren(createMutedParagraph("Aucun trajet diffuse actuellement."));
  } else {
    liveList.replaceChildren(
      ...liveTrips.map((trip) => {
        ...
        const button = document.createElement("button");
        button.className = "map-live-item";
        button.type = "button";
        button.dataset.mapLine = text(trip.lineCode);

        const pillWrap = document.createElement("span");
        pillWrap.appendChild(createLinePill(trip.displayCode, trip.colorValue));
        const strong = document.createElement("strong");
        strong.textContent = text(trip.ownerName || "Trajet live");
        const small = document.createElement("small");
        small.textContent = `${number(trip.speedKmh, 1)} km/h - ${number(trip.observers)} observateur(s) - ${text(status)}`;
        button.append(pillWrap, strong, small);
        return button;
      }),
    );
  }
  ...
  if (showUsersValue && liveUsers.length) {
    userList.replaceChildren(
      ...liveUsers.map((user) => {
        ...
        const item = document.createElement("div");
        item.className = "map-live-item";

        const pillWrap = document.createElement("span");
        pillWrap.appendChild(createLinePill(user.displayCode || "U", "#0f8b8d"));
        const strong = document.createElement("strong");
        strong.textContent = text(user.fullName || user.name || user.email || "Utilisateur");
        const small = document.createElement("small");
        small.textContent = `${text(age)}${location.lat != null && location.lng != null ? " - GPS" : ""}`;
        item.append(pillWrap, strong, small);
        return item;
      }),
    );
  } else {
    userList.replaceChildren(createMutedParagraph("Aucun utilisateur visible pour le moment."));
  }

  const legend = $("mapLegend");
  if (!visibleLines.length) {
    legend.replaceChildren(createMutedParagraph("Aucune ligne visible."));
    return;
  }
  legend.replaceChildren(
    ...visibleLines.slice(0, 18).map((line) => {
      ...
      button.type = "button";
      button.dataset.mapLine = text(line.lineCode);

      const swatch = document.createElement("span");
      swatch.style.background = color(line.colorValue);
      const strong = document.createElement("strong");
      strong.textContent = text(line.displayCode);
      const small = document.createElement("small");
      small.textContent = text(line.title || "Itineraire");
...
function drawAdminMap(canvas, lines, liveTrips, liveUsers, showUsers) {
...
function handleMapPointer(event) {
  ...
  if (marker.type === "user") {
    ...
    renderHtml(tooltip, `
      <strong>${escapeHtml(user.fullName || user.name || user.email || "Utilisateur")}</strong>
      <span>${escapeHtml(user.role || "utilisateur")}</span>
      <span>GPS ${escapeHtml(gpsAgeLabel(user.location?.timestamp))}</span>
    `);
  } else {
    const trip = marker.trip;
    renderHtml(tooltip, `
      <strong>${escapeHtml(trip.displayCode)} - ${escapeHtml(trip.ownerName)}</strong>
      <span>${number(trip.speedKmh, 1)} km/h - ${number(trip.observers)} observateur(s)</span>
      <span>GPS ${escapeHtml(gpsAgeLabel(trip.liveLocation?.timestamp || trip.lastUpdatedAt))}</span>
    `);
  }
...
function renderEvents(events) {
  ...
  renderHtml($("eventsList"),
    events
      .map(
        (event) => `
          <div class="event-item" style="border-left-color:${color(event.colorValue)}">
            <strong>${escapeHtml(event.title)}</strong>
            <span>${escapeHtml(event.subtitle)}</span>
            <span>${dateTime(event.timestamp)} - ${escapeHtml(event.iconKey)}</span>
          </div>
        `,
      )
...
function linePill(label, colorValue) {
  return `<span class="line-pill" style="background:${color(colorValue)}">${escapeHtml(label || "-")}</span>`;
}
    ...
    <tr class="trip-detail-row">
      <td colspan="${colspan}">
        <div class="trip-detail-card">
          ...
          <div class="trip-detail-note">
            <strong>Note trajet</strong>
            <span>${escapeHtml(text(trip.note, "Aucune note."))}</span>
          </div>
          <div class="trip-detail-note">
            <strong>Dernier message systeme</strong>
            <span>${escapeHtml(text(trip.lastSystemMessage, "Aucun message systeme."))}</span>
          </div>
          <div class="trip-chat">
            <div class="trip-chat-title">
              <strong>Chat du trajet</strong>
              <span>${escapeHtml(chatSubtitle)}</span>
            </div>
            ${tripMessagesHtml(messages)}
          </div>
        </div>
      </td>
    </tr>
...
function tripMessagesHtml(messages) {
  if (!messages.length) {
    return `<p class="muted-text">Aucun message archive pour ce trajet.</p>`;
  }
  return messages
    .map(
      (message) => `
        <div class="trip-message ${message.isSystem ? "is-system" : ""}">
          <div>
            <strong>${escapeHtml(text(message.authorName, "Systeme"))}</strong>
            <small>${escapeHtml(text(message.role, "-"))} - ${dateTime(message.createdAt)}</small>
          </div>
